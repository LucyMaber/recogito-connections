import { G, Container } from '@svgdotjs/svg.js';
...
export default class SVGFloatingEdge {
  ...
  start: NetworkNode;
  end: CoordinatePoint | NetworkNode | null;
  svg: Container;
  g: G;
  hidden: boolean;
  
  constructor(startNode: NetworkNode, svg: Container) {
  ...
  isSnapped = (): boolean =>
    !!(this.end && (this.end as any).annotation);

  redraw = (): void => {
    window.requestAnimationFrame(() => {
      if (this.end) {
        const start = this.start.getAttachableRect();
        const end = (this.end as any).annotation ? (this.end as NetworkNode).getAttachableRect() : (this.end as CoordinatePoint);
        ...
        const [ sx, sy, cx, cy, ex, ey, ae, ] = getBoxToBoxArrow(
          ...
          ('width' in end ? end.width : 0),
          ('height' in end ? end.height : 0),
          ARROW_CONFIG
        );
